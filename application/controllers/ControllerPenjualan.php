...
class ControllerPenjualan extends CI_Controller
{
    ...
    public function cetak()
    {
        ...
        $pdf->Cell(10,1,'',0,1);
        $pdf->Cell(190,10,'LAPORAN PENJUALAN TANGGAL '.shortdate_indo($awal). ' SAMPAI '.shortdate_indo($akhir),0,1,'C');
        
        $pdf->Cell(10,-1,'',0,1);

        $pendapatan = $this->Model->lapPenjualan($awal,$akhir);
        ...
        foreach ($pendapatan as $row)
        {
            ...
            $pdf->Cell(45,6,number_format($row->jml_bayar,0,',','.'),1,1,'C');
        }

        $grand_total = $this->Model->lapPenjualan_diskon($awal,$akhir);
        ...
        $pdf->SetFont('Arial','B',8);
        $pdf->Cell(145,6,'Diskon',1,0,'C');

        if(array_sum($diskon) == 0){
            $pdf->Cell(45,6,'-',1,1,'C');
        } else {
            $pdf->Cell(45,6,'Rp. '.number_format(array_sum($diskon),0,',','.'),1,1,'C');
        }
